...
- Реальная отправка сообщений через Telegram Bot API и webhook (требуется `TELEGRAM_BOT_TOKEN`).
- Email-рассылки и платежи по сделкам — отложены на следующие итерации.
...
1. Добавить email-рассылки и платежи по сделкам (на финальной стадии).
2. Углублённая интеграция CRM ↔ проекты: автосоздание сделки из проекта, КП, счета из сделки.

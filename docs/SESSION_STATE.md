...
Последний коммит: `8e07a7a` — `ui(cables): single-row layout per cable in panel`.
...
- Проверено в браузере: в меню **Панели** отдельно **Кабельный журнал** и **Кабели**; панель **Кабели** показывает кабели текущего листа в одну строку: флажок, маркировка, марка, длина; выделение строки подсвечивает кабель.

...
Последний коммит: `36846b4` — `ui(cables): brand and cross-section dropdowns`.
...
- `src/components/editor/PanelManager.ts`:
  - `PanelConfig` получил опциональное поле `width`; ширина панели учитывается при начальной позиции и сбросе раскладки.
- `src/app/globals.css`:
...
- В меню **Панели** (нижний dock) теперь два отдельных пункта: **Кабельный журнал** и **Кабели**.

### Проверки
- `npx tsc --noEmit` — чисто.
- `npm test` — 39/39.
- Dev-сервер запущен на `http://localhost:3002/editor`.
- Проверено в браузере: панель **Кабели** шириной 400px; одна строка на кабель: флажок, маркировка группы, марка кабеля (выпадающий список), сечение (выпадающий список), длина до десятых; марка `КГВВГнг(А)-FRLS` помещается.

...
- `packages/core/render/SheetFrameRenderer.ts`:
  ...
  - Добавлено поле **Компания**:
    ...
    - Исправлено закрытие меню «Штамп» при выборе файла: обработчик внешнего клика использует `mousedown` и флаг `isChoosingFile`, поэтому меню остаётся открытым после закрытия системного диалога выбора файла.

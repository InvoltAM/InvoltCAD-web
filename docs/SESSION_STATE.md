...
- `packages/core/tools/SelectTool.ts` — drag вершин/граней кабеля теперь:
  ...
  - зоны hit-test кабеля, вершин и рёбер увеличены (24/12/12 мм), чтобы кабель было проще выделить.
...
### Валидация кабелей по спецификации v3.1 (фаза 2)

- `packages/core/cables/CableValidator.ts` — полный `CableValidator`:
  - `validateCable` c режимами `strict` / `user`;
  - проверка отступа 400 мм от поверхности стены (100 мм для connector-сегментов);
  - Safe Zone 50×50 см вокруг блока исключает ложные нарушения у устройств;
  - проверка пересечения со стеной (разрешено через дверной проём);
  - проверка Doorway Restricted Zone (10 см от краёв проёма);
  - проверка, что сегмент не проходит внутри стены (в материале стены).
- `packages/core/cables/WallCollisionResolver.ts` — `resolvePoint` / `resolveSegment`:
  - блокирует точку внутри стены;
  - clamp'ит точку к поверхности + отступ (400/100 мм);
  - останавливает сегмент у стены с запасом 5 см.
- `packages/core/rules/CableRules.ts` — `validateCables` вызывает `validateCable` и добавляет issue при геометрических нарушениях.
- `packages/core/render/CableRenderer.ts` — для выделенного кабеля рисуются красные маркеры нарушений.
- `packages/core/tools/CableTool.ts` — интегрирован `WallCollisionResolver`:
  - клик внутри стены игнорируется;
  - ghost-линия меняет цвет при блокировке;
  - промежуточные узлы тоже проходят проверку.
- Тесты:
  - `packages/core/cables/CableValidator.test.ts` — 6 тестов (валидный кабель, пересечение, дверной проём, отступ 400 мм, connector offset, user mode).
  - Исправлен `packages/core/tools/CableTool.test.ts` — точка "на стене" заменена на точку в комнате, чтобы не нарушать новое правило "внутри стены".
- Проверки:
  - `npx tsc --noEmit` — чисто.
  - `npm test -- --run` — 19 файлов, **101 тест** пройден.

## Следующие шаги

- Продолжить выполнение спецификации `D:\Kimi BAZA_CRM\cable_routing_spec.md`:
  - фаза 3 — Visio-style drag сегментов с автообходом препятствий и ортогональным перемещением;
  - фаза 4 — multi-cable bundle (trunk / parallel) и стили кабеля (цвет, толщина, пунктир).
...
- Добавлен масштаб (zoom) в редактор визуализации щита (`src/components/editor/PanelEditor.tsx`):
  - `Ctrl + колесо мыши` изменяет масштаб рабочей области с шагом ±10%.
  - Диапазон масштаба ограничен 50%–300%.
  - В правом нижнем углу добавлена плавающая панель с кнопками `−`, `+`, `↺` и текущим процентом масштаба.
  - Весь контент основной области (визуализация реек и вкладка «Набор») обёрнут в `transform: scale(zoom)`, начало трансформации — `top left`.
  - Drag-and-drop элементов на рейках продолжает работать при любом масштабе.
  - Масштабирование изолировано от зума страницы браузера: нативный обработчик `wheel` с `passive: false` на окне редактора вызывает `preventDefault()` при зажатом `Ctrl`, чтобы не масштабировалась вся страница.
- Окно «Визуализация щита» теперь можно растягивать мышью:
  - Ручки изменения размера добавлены в правом нижнем и верхнем левом углах окна (`nwse-resize`).
  - Размер ограничен: минимум 640×400 px, максимум 95% viewport по ширине/высоте.
  - При открытии окно сохраняет прежние пропорции (90 vw / 80 vh, max-width 1024 px).
  - Внутренний контент и панель масштаба корректно подстраиваются под новый размер.
- Окно «Визуализация щита» теперь можно перетаскивать за заголовок:
  - Зажатие левой кнопкой мыши по верхней панели (кроме кнопки закрытия) перемещает окно.
  - Перемещение реализовано через глобальные `pointermove`/`pointerup`.
- Приведены размеры реек в соответствие со стандартом:
  - Ширина модуля: 18 мм, ширина рейки на 12 модулей: 216 мм.
  - Высота рейки и устройств (автоматы, УЗО, диф. автоматы, контакторы): 80 мм.
  - Расстояние между центральными осями реек: 125 мм.
  - Размеры устройств и реек заданы в миллиметрах (`mm`) через inline-стили.
- Проверки:
  - `npx tsc --noEmit` — чисто.
  - `npm test -- --run` — 63/63 тестов пройдены.
  - В браузере: zoom панели и перетаскивание аппаратов работают корректно.
...
# Сессия разработки — 2026-08-21 (редактор визуализации щита)
...
- Исправлено перетаскивание и удаление элементов во вкладке «Визуализация щита» (`src/components/editor/PanelEditor.tsx`):
  - Drag-and-drop переведён на глобальные `pointermove`/`pointerup` с `setPointerCapture`, чтобы события не терялись.
  - Удаление работает с подтверждением и для щита из однолинейной схемы, и для fallback-генерации.
- Добавлена возможность редактировать fallback-визуализацию щита без однолинейной схемы:
  - Хранится порядок устройств и скрытые/удалённые элементы.
- Окно «Визуализация щита» теперь закрывается только по крестику в углу.
- Добавлена кнопка «+ Добавить рейку» под рейками — добавляет пустые рейки ниже существующих.
- На каждой рейке появилась кнопка «+», размещённая на самой рейке справа от последнего аппарата; при добавлении устройств кнопка сдвигается за ними.
- При нажатии на «+» открывается выпадающее меню выбора аппаратов:
  - Автоматы 16/25/32/40 А
  - УЗО 25/40/63 А 30 мА
  - Контактор 25 А
  - Шины N и PE
  - Заглушка (пустая плашка 1 модуль)
- Добавленные устройства сохраняются в `board.components` (для ОЛС) или в локальном состоянии fallback-визуализации.
- Добавлена кнопка «+ Добавить отсек» — создаёт вторую и последующие колонки DIN-реек шириной 12 модулей.
- Устройства корректно добавляются в выбранный отсек; перетаскивание реализовано между отсеками через `moveIdInSection`.
- Добавлена возможность удалять рейки и отсеки:
  - Рядом с заголовком «Рейка N» появилась кнопка «×» — удаляет все аппараты на этой рейке (с подтверждением) или убирает пустую добавленную рейку.
  - Рядом с заголовком «Отсек N» появилась кнопка «×» — удаляет весь отсек со всеми устройствами (с подтверждением); последний отсек удалить нельзя.
- Все изменения прошли `npx tsc --noEmit` и `npm test -- --run` (63/63 тестов).
- Коммиты:
  - `248b8e2` — `feat(panel): добавлены отсеки (колонки DIN-реек) в визуализации щита`
  - `9492f86` — `feat(panel): возможность удалять рейки и отсеки в визуализации щита`
  - `92c724f` — `fix(panel): одинаковая ширина отсеков в визуализации щита`
...
- Кнопка «Меню» перенесена в самый верх левой панели:
  - `src/components/editor/Toolbar.tsx` — добавлена первая кнопка «Меню» в `project-sidebar-top`; по клику возвращает на стартовую страницу (`/`).
  - Удалён отдельный компонент `EditorMenuButton` и его стили.
...
- В редакторе щита (`PanelEditor`) добавлена левая панель с кнопками:
  - `src/components/editor/PanelEditor.tsx` — внутри модального окна «Визуализация щита» добавлена левая колонка с кнопками «Щит» (переключение на визуализацию) и «Корзина» (переключение на список элементов).
  - «Корзина» отображает элементы щита из `plan.electrical.distributionBoards[0].components`, собранные из однолинейной схемы: название, тип, ширина в модулях, итоговое число модулей.
  - Реализовано перетаскивание элементов на DIN-рейках мышью: устройства можно переносить между позициями и между рейками; порядок синхронизируется с `board.components`.
  - Добавлено удаление элементов с реек с подтверждением: кнопка `×` появляется при наведении на устройство; после подтверждения элемент удаляется из `board.components`.
...
- Созданы страницы CRM:
  - `/crm/clients` — таблица клиентов с удалением.
  - `/crm/clients/new` — форма создания клиента.
  - `/crm/leads` — таблица лидов с удалением.
  - `/crm/leads/new` — форма создания лида.
- Проверено в браузере: страницы открываются, неавторизованный пользователь перенаправляется на `/login` с нужным `callbackUrl`.
- Созданы страницы редактирования:
  - `/crm/clients/[id]` — редактирование клиента.
  - `/crm/leads/[id]` — редактирование лида.
- Строки в таблицах клиентов и лидов теперь кликабельны и ведут на страницу редактирования.
- Проверки пройдены:
  - `npx tsc --noEmit` — успешно.
  - `npm test -- --run` — 63 теста успешно.
...
- Страница календаря `/crm/calendar` с месячным видом, модальным окном создания/редактирования событий.
- API активности `/api/crm/activity`.
- Страница истории активности `/crm/activity`.
- Дашборд CRM дополнен карточками календаря и кнопками быстрого создания клиента/лида/сделки/задачи, ссылкой на историю активности.
- API Telegram: `GET/POST /api/crm/telegram`, `POST /api/crm/telegram/send`. Сообщения сохраняются в `CrmTelegramLog`. Реальная отправка через Telegram Bot API отложена.
- Вкладка Telegram на страницах `/crm/clients/[id]` и `/crm/leads/[id]`: история сообщений и форма отправки.
- В модель `CrmClient` добавлено поле `telegramChatId`.
- Интеграция CRM с проектами:
  - В схему `Project` добавлены поля `crmClientId`, `crmDealId` и связи с `CrmClient`, `CrmDeal`.
  - API проектов поддерживает CRM-связи в `POST /api/projects`, `GET /api/projects/[id]`, `PATCH /api/projects/[id]`.
  - В панели `ProjectsPanel` добавлена кнопка «CRM» для привязки проекта к клиенту и сделке.
  - На страницах клиента и сделки (`/crm/clients/[id]`, `/crm/deals/[id]`) отображаются связанные проекты со ссылками в редактор.

## Что ещё не сделано

### Email-рассылки, платежи и интеграция CRM ↔ проекты

- Расширена схема Prisma:
  - Добавлена модель `CrmEmailLog` (получатель, тема, тело, статус, ошибка, связь со сделкой/клиентом/лидом).
  - В `Payment` добавлено поле `crmDealId` и цель `purpose='deal'`.
  - В `Estimate`, `Invoice`, `Document` добавлено поле `crmDealId`.
  - `CrmDeal` получила связи `emails`, `payments`, `estimates`, `invoices`, `documents`.
  - Схема синхронизирована через `npx prisma db push`, клиент перегенерирован.
- Создан сервис отправки email `src/lib/email.ts` на базе `nodemailer` с использованием существующих `EMAIL_SERVER_*` переменных.
- Добавлен API email:
  - `GET /api/crm/emails` — история писем по сделке/клиенту/лиду.
  - `POST /api/crm/emails/send` — отправка письма. Если SMTP не настроен, письмо сохраняется в лог со статусом `failed` и пояснением.
- Добавлен API платежей по сделке:
  - `GET /api/crm/deals/[id]/payments` — список платежей.
  - `POST /api/crm/deals/[id]/payments` — создание YooKassa-платежа на сумму сделки (или заданную), возвращает ссылку на оплату.
- Обновлён webhook YooKassa `src/app/api/billing/webhook/route.ts`:
  - При `purpose='deal'` и успешной оплате сделка переходит в стадию `won`, фиксируется `closedAt`, пишется запись в `CrmActivityLog`.
- Исправлен `GET /api/crm/deals/[id]` — теперь возвращает сделку с клиентом (ранее endpoint отсутствовал, страница сделки использовала его).
- Обновлена страница сделки `src/app/crm/deals/[id]/page.tsx`:
  - Панель **Email** — форма отправки письма клиенту с автозаполнением email, история переписки.
  - Панель **Платежи** — кнопка «Создать ссылку на оплату» на сумму сделки, список платежей со статусами.
  - Панели **КП**, **Счета**, **Документы** — списки и кнопки создания из сделки.
- Добавлена интеграция CRM ↔ проекты:
  - `POST /api/projects/[id]/deal` — автосоздание сделки из проекта с привязкой к проекту.
  - `GET/POST /api/crm/deals/[id]/estimates` — КП по сделке; создание КП на сумму сделки.
  - `GET/POST /api/crm/deals/[id]/invoices` — счета по сделке; создание счёта на сумму сделки.
  - `GET /api/crm/deals/[id]/documents` — документы по сделке.
  - `PUT /api/projects/[id]` сохраняет `crmDealId` у estimates/invoices/documents, чтобы связь не терялась при сохранении плана.
  - В `ProjectsPanel` в CRM-диалог добавлена кнопка «＋ Создать сделку из проекта».
- Проверки:
  - `npx tsc --noEmit` — успешно.
  - `npm test -- --run` — 63 теста успешно.

### Шаблоны email, массовые рассылки и отправка КП/счетов по email

- Расширена схема Prisma:
  - Добавлена модель `CrmEmailTemplate` (название, тема, тело, связь с пользователем).
  - В `CrmEmailLog` добавлено поле `templateId` для отслеживания использования шаблонов.
  - Схема синхронизирована через `npx prisma db push`, клиент перегенерирован.
- API шаблонов email:
  - `GET /api/crm/email-templates` — список шаблонов.
  - `POST /api/crm/email-templates` — создание.
  - `PATCH /api/crm/email-templates/[id]` — редактирование.
  - `DELETE /api/crm/email-templates/[id]` — удаление.
- API массовой рассылки:
  - `POST /api/crm/emails/bulk` — массовая отправка по списку получателей (клиенты/лиды).
- API отправки КП/счетов по email:
  - `POST /api/crm/deals/[id]/estimates/[estimateId]/email` — публикует КП (генерирует `publicSlug`) и отправляет ссылку.
  - `POST /api/crm/deals/[id]/invoices/[invoiceId]/email` — отправляет email со счётом.
- UI:
  - Страница `/crm/email-templates` — управление шаблонами (создание, редактирование, удаление).
  - Страница `/crm/email-blast` — массовая рассылка с выбором получателей и шаблона.
  - В панели **Email** на странице сделки появился select с шаблонами и ссылка на управление шаблонами.
  - В панелях **КП** и **Счета** на странице сделки добавлены кнопки «Отправить».
  - В CRM-дашборд добавлены кнопки «Шаблоны email» и «Массовая рассылка».
- Проверки:
  - `npx tsc --noEmit` — успешно.
  - `npm test -- --run` — 63 теста успешно.

### Напоминания по задачам CRM

- Расширена схема Prisma:
  - В `CrmTask` добавлены поля `reminderAt` (дата/время напоминания) и `reminderSent` (флаг отправки).
  - Схема синхронизирована через `npx prisma db push`, клиент перегенерирован.
- API напоминаний:
  - `GET /api/crm/tasks/reminders` — список задач с подошедшим сроком напоминания.
  - `POST /api/crm/tasks/reminders` — отправляет email-напоминания пользователю по задачам с `reminderAt <= now` и `reminderSent = false`, отмечает их `reminderSent = true`.
- Обновлены API задач:
  - `POST /api/crm/tasks` и `PATCH /api/crm/tasks/[id]` поддерживают `reminderAt`.
  - При изменении `reminderAt` сбрасывается `reminderSent`, чтобы напоминание отправилось повторно.
- UI:
  - В формах создания и редактирования задачи добавлены поля «Напомнить (дата/время)».
  - В CRM-дашборд добавлена кнопка **«Проверить напоминания»** (`CheckRemindersButton`), которая вызывает API и показывает результат.
- Проверки:
  - `npx tsc --noEmit` — успешно.
  - `npm test -- --run` — 63 теста успешно.

### Реальная отправка Telegram и webhook

- Добавлена переменная окружения `TELEGRAM_BOT_TOKEN` в `.env.example`.
- Обновлён `src/app/api/crm/telegram/send/route.ts`:
  - Если `TELEGRAM_BOT_TOKEN` задан, выполняется реальный `fetch` к `https://api.telegram.org/bot<TOKEN>/sendMessage`.
  - Сообщения отправляются с `parse_mode: 'HTML'`.
  - Результат отправки фиксируется в поле `status` (`sent` / `failed`) и `errorMessage`.
  - Без токена сохраняется прежнее поведение — запись в лог с пояснением.
  - Поддерживаются `leadId` и `clientId` для проверки прав доступа.
- Создан обработчик webhook `src/app/api/crm/telegram/webhook/route.ts`:
  - Принимает входящие обновления от Telegram Bot API.
  - Ищет клиента или лида по `telegramChatId`.
  - Сохраняет входящее текстовое сообщение в `CrmTelegramLog`.
- Исправлен `GET /api/crm/telegram`:
  - Фильтр по `clientId` теперь ищет `telegramChatId` клиента, а не использует `clientId` как `chatId`.
  - Добавлена проверка доступа для лидов и клиентов.
- Обновлены страницы клиента и лида: при отправке передаётся `clientId`/`leadId`; при неудачной отправке показывается пояснение.
- Проверки:
  - `npx tsc --noEmit` — успешно.
  - `npm test -- --run` — 63 теста успешно.
...
1. Воронка продаж: визуальная воронка, статусы сделок, conversion rate.
2. Редактирование связанных КП/счетов/документов прямо на странице сделки.
3. Автоматический cron для проверки напоминаний по задачам (сейчас ручная кнопка).
4. Запустить `npx tsc --noEmit` и `npm test -- --run`, исправить ошибки.
5. Закоммитить очередной рабочий срез.
6. Обновить `docs/SESSION_STATE.md`.

---

# Сессия разработки — 2026-08-21 (воронка продаж)
...
- Создан API воронки продаж:
  - `GET /api/crm/deals/funnel` — группирует сделки пользователя по статусам (`new`, `qualified`, `proposal`, `negotiation`, `won`, `lost`).
  - `PATCH /api/crm/deals/funnel` — обновляет статус сделки по `dealId` (drag-and-drop).
- Создана страница воронки `/crm/funnel/page.tsx`:
  - Колонки для каждого статуса.
  - Drag-and-drop карточек сделок между колонками.
  - Итоговая сумма по каждой колонке.
  - Ссылка на страницу сделки.
- В CRM-дашборд `src/app/crm/page.tsx` добавлена кнопка «Воронка продаж» рядом с кнопками «Шаблоны email», «Массовая рассылка», «Проверить напоминания».
- Проверки:
  - `npx tsc --noEmit` — успешно.
  - `npm test -- --run` — 63 теста успешно.
...
# Сессия разработки — 2026-08-21 (редактирование КП/счетов/документов в сделке)
...
- Добавлено редактирование связанных КП/счетов/документов прямо на странице сделки `src/app/crm/deals/[id]/page.tsx`:
  - Панель **Коммерческие предложения**: inline-редактирование названия, суммы и статуса; удаление с подтверждением.
  - Панель **Счета**: inline-редактирование номера, суммы и статуса; удаление с подтверждением.
  - Панель **Документы**: создание документа из сделки, inline-редактирование названия, типа и статуса; удаление с подтверждением.
- API:
  - `PUT /api/crm/deals/[id]/estimates/[estimateId]` — обновить КП (название, сумма, статус).
  - `DELETE /api/crm/deals/[id]/estimates/[estimateId]` — удалить КП.
  - `PUT /api/crm/deals/[id]/invoices/[invoiceId]` — обновить счёт (номер, сумма, статус).
  - `DELETE /api/crm/deals/[id]/invoices/[invoiceId]` — удалить счёт.
  - `POST /api/crm/deals/[id]/documents` — создать документ из сделки.
  - `PUT /api/crm/deals/[id]/documents/[documentId]` — обновить документ (название, тип, статус).
...
# Сессия разработки — 2026-08-21 (cron для напоминаний по задачам CRM)
...
- Создан cron-эндпоинт `src/app/api/cron/crm-task-reminders/route.ts`:
  - `GET/POST /api/cron/crm-task-reminders` — обходит всех пользователей с `email` и отправляет напоминания по задачам CRM с `reminderAt <= now` и `reminderSent = false`.
  - Защищён секретом `CRON_SECRET` (или `VERCEL_CRON_SECRET`) через заголовок `Authorization: Bearer <secret>`.
  - Если SMTP не настроен, возвращает `skipped: true` без отправки.
  - После успешной отправки обновляет `reminderSent = true` и пишет запись в `CrmActivityLog`.
- Добавлена переменная `CRON_SECRET` в `.env.example`.
- Для автоматизации можно настроить внешний cron (например, Vercel Cron, GitHub Actions или системный cron) на вызов эндпоинта каждые 5–15 минут.
- Проверки:
  - `npx tsc --noEmit` — успешно.
  - `npm test -- --run` — 63 теста успешно.
...
# Сессия разработки — 2026-08-21 (тестовый вход для CRM)
...
- Добавлен тестовый вход через Credentials-провайдер в `src/lib/auth.ts` (только в `development`):
  - Пароль: `password`.
  - Если пользователь с введённым email не существует, он автоматически создаётся с ролью `admin` и 1000 кредитов.
  - Авторизация переведена на JWT-сессии, чтобы Credentials-провайдер работал корректно.
  - В `session` callback загружаются `role` и `credits` из БД по `token.sub`.
- Обновлена страница входа `src/app/login/page.tsx`:
  - В dev-режиме под формой email появляется блок «тестовый вход» с полями email/пароль.
- Проверки:
  - `npx tsc --noEmit` — успешно.
  - `npm test -- --run` — 63 теста успешно.

## Как зайти в CRM для тестирования

1. Открыть `http://localhost:3003`.
2. Нажать **CRM** в главном меню → редирект на `/login?callbackUrl=/crm`.
3. В блоке «тестовый вход» ввести любой email (например, `admin@example.com`) и пароль `password`.
4. После входа попадаешь в CRM-дашборд `/crm`.

## Что ещё не сделано

1. Продолжить по плану CRM / перейти к следующей фиче из глобального плана разработки.
...
# Сессия разработки — 2026-08-22 (PanelEditor: удаление и drag-to-right)

## Текущий контекст

Работа ведётся в репозитории **InvoltCAD-web**, ветка `main`.
Dev-сервер Next.js работает на `http://localhost:3002`.

## Что сделано в текущей сессии

- Исправлено удаление устройств в редакторе визуализации щита (`src/components/editor/PanelEditor.tsx`):
  - В fallback-режиме (нет собранной однолинейной схемы) устройство теперь удаляется из `sectionOrders` и заносится в `hidden`.
  - Fallback-эффект синхронизации больше не возвращает скрытые базовые устройства обратно на рейку.
  - Рендеринг fallback-групп фильтрует скрытые (`hidden`) id.
  - Проверено в браузере: кнопка «×» на устройстве после подтверждения удаляет его из рейки.

- Исправлено перетаскивание устройства в самую правую позицию на рейке (`src/components/editor/PanelEditor.tsx`):
  - `getDropTargetSection` исключает перетаскиваемое устройство из DOM-списка, поэтому вычисленный `targetIndex` уже учитывает отсутствие источника.
  - Убрана лишняя корректировка `targetIndex - 1` в `moveIdInSection`, из-за которой блок вставлялся на одну позицию левее конца.
  - Проверено в браузере: устройства перетаскиваются и во вторую справа, и в самую правую позицию.

## Коммиты

- `9ef9164` — fix(PanelEditor): удаление устройств в fallback-режиме редактора щита
- `cc19e5a` — fix(PanelEditor): перетаскивание устройства в самую правую позицию рейки
...
# Сессия разработки — 2026-08-22 (OlsPanel: схема/таблица/спецификация + элементы, default tool)
...
- В `OlsPanel` (открывается по кнопке **ОЛС** в основной левой панели редактора) добавлена левая панель с вкладками:
  - **Схема** — отображает существующую SVG-однолинейную схему / щит.
  - **Таблица щита** — таблица устройств собранного щита (№, наименование, тип, Iном, модули).
  - **Спецификация щита** — группировка одинаковых устройств с подсчётом количества (№, наименование, тип, Iном, модули, кол-во).
- Добавлен раздел **«Элементы»** в левую панель `OlsPanel` с кнопками:
  - Автоматы, Контакторы, УЗО, Дифы, Реле, УД.
  - Клик выбирает элемент (повторный клик снимает выделение); функционал размещения элементов на схеме — на следующих итерациях.
- Убрана вкладка «ОЛС» из модального окна редактора щита (`PanelEditor`) — функционал перенесён в основную панель `OlsPanel`.
- Изменён базовый инструмент при входе в редактор (`src/stores/cadStore.ts`):
  - `currentTool` по умолчанию теперь `'select'` вместо `'wall'`.
  - При загрузке редактора активен инструмент **«Выбор»**.

## Коммиты

- `7a2b7cf` — refactor: перенос вкладки ОЛС из PanelEditor в основную панель OlsPanel
- `1edab6d` — feat(OlsPanel): кнопка 'Спецификация щита' с группировкой по позициям
- `1cb36f0` — feat(OlsPanel): панель элементов ОЛС — Автоматы, Контакторы, УЗО, Дифы, Реле, УД
- `870025f` — feat: базовый инструмент при входе в редактор — 'выделить'
...
## 2026-08-23 (продолжение) — Кабель: минимум вершин, прямые углы, обход перегородок

### Изменённые файлы

- `packages/core/cables/cableRouting.ts`
  - Добавлена `postprocessRoute`: выравнивает отрезки под прямые углами, удаляет избыточные вершины и прижимает маршрут к параллельным стенам с зазором.
  - `straightenRoute` теперь обрабатывает и 45°-отрезки, превращая их в горизонтальные сегменты, чтобы маршрут состоял только из прямых углов.
  - `routeCable` применяет `postprocessRoute` к результату A*.
- `packages/core/cables/cableRouting.test.ts`
  - Добавлен тест `обходит вертикальную перегородку между устройствами на одной стене` с дверным проёмом.

### Проверки

- `npx tsc --noEmit` — успешно.
- `npm test` — 82/82 теста проходят.
- В браузере (Playwright) проверено:
  - Автотрассировка между двумя розетками на противоположных сторонах перегородки проходит через дверной проём.
  - Маршрут состоит из 4 вершин, все повороты — прямые углы, диагональных сегментов нет.
  - Кабель выделяется инструментом «Выбор», доступны drag вершин/граней, двойной клик для добавления/удаления вершин.

## 2026-08-23 (продолжение) — Кабель: прямоугольный соединитель MS Visio, минимум вершин

### Цель
Сделать автотрассировку кабеля похожей на прямоугольный соединитель MS Visio: минимальное число вершин, только прямые углы, маршрут параллелен стенам, без «лесенки».

### Изменённые файлы

- `packages/core/cables/cableRouting.ts`
  - Добавлен `orthogonalDPSimplify`: динамическое программирование по подмножеству исходных вершин, выбирает маршрут с минимальным числом горизонтальных/вертикальных сегментов (прямой отрезок или L-образный излом), если отрезки не пересекают стены вне проёмов и сохраняют зазор `WALL_CLEARANCE`.
  - Добавлен `insertOpeningWaypoints`: вставляет центры проёмов в маршрут, чтобы DP могла коротко проходить через дверные/оконные проёмы.
  - Добавлен `snapCrossingsToOpenings`: сдвигает сегменты, пересекающие стены через проёмы, к центральной линии проёма.
  - Добавлены `mergeCollinearSegments` и `removeRedundantVertices`: дополнительно удаляют лишние вершины, если прямой отрезок между соседними точками допустим.
  - `postprocessRoute` теперь вызывает цепочку: `straightenRoute` → `deduplicateRoute` → `insertOpeningWaypoints` → `orthogonalDPSimplify` → `snapCrossingsToOpenings` → `mergeCollinearSegments` → `removeRedundantVertices` → повторное `straightenRoute`/`mergeCollinearSegments`/`removeRedundantVertices`.
  - `segmentCrossesWallOutsideOpening` и `snapCrossingToOpening` вынесены и переиспользуются при проверках.
- `packages/core/cables/cableRouting.test.ts`  - Добавлен тест `typical socket layouts produce compact Visio-like routes` с asserts: маршрут по одной стене с перегородкой ≤ 5 вершин, маршрут left-right ≤ 4 вершин, проход через проём обязателен.

### Проверки

- `npx tsc --noEmit` — успешно.
- `npm test` — 83/83 теста проходят.
- В браузере (Playwright) проверено:  - Автотрассировка между двумя розетками в комнате 6×4 м: 3 вершины, два ортогональных отрезка, без диагоналей и лишних изгибов.  - Автотрассировка через центральную перегородку с дверным проёмом: 2 вершины, прямой горизонтальный отрезок проходит через проём.

## 2026-08-23 (продолжение) — Кабель: только дверные проёмы, окна как стены

### Изменённые файлы

- `packages/core/cables/cableRouting.ts`
  - `insertOpeningWaypoints` добавляет вспомогательные точки только для проёмов `type === 'door'`.
  - `findWallOpeningCrossing` считает пересечение допустимым только для дверных проёмов; оконные проёмы трактуются как стена.
- `packages/core/cables/navGrid.ts`
  - `NavGrid.fromPlan` очищает проходимые ячейки только для `opening.type === 'door'`; окна остаются непроходимыми.
- `packages/core/cables/cableRouting.test.ts`  - Добавлен тест `не проходит через оконный проём`: перегородка с окном полностью разделяет комнату, маршрут не строится.
...
## 2026-08-23 (продолжение) — Редактирование кабеля: запрет пересечения стен

### Изменённые файлы

- `packages/core/tools/CableEditHelper.ts`
  - Добавлены `cableRouteHasWallCrossing` и `repairCableRoute`.
  - `repairCableRoute` проходит по всем отрезкам маршрута и заменяет пересекающие стены (вне дверных проёмов) на автотрассированные обходы.
  - Если обойти стену невозможно, `repaired === false`.
- `packages/core/tools/SelectTool.ts`
  - При перемещении вершины и грани кабеля вызывается `repairCableRoute`.
  - Если маршрут починить удалось — применяется починенный вариант.
  - Если нет — маршрут откатывается к исходному, и кабель не пересекает стену.
- `packages/core/tools/SelectTool.cableEdit.test.ts`
  - Добавлены тесты: запрет пересечения стены при перемещении вершины, автоматический обход через дверной проём, откат к исходному маршруту, если обхода нет.
...
## 2026-08-23 (продолжение) — Fillet-рендеринг и устранение зависаний при редактировании кабеля

### Изменённые файлы

- `packages/core/render/CableRenderer.ts`
  - Добавлен приватный метод `drawCablePath`, рисующий кабель со скруглёнными углами (fillet) радиусом 40 мм.
  - Скругление применяется только к прямым углам; коллинеарные сегменты остаются прямыми.
  - Радиус ограничен 35 % от длины каждого смежного сегмента.
  - Увеличена толщина линии до 5/7 экранных пикселей для лучшей видимости.
- `packages/core/tools/SelectTool.ts`
  - При drag вершины/грани кабеля `repairCableRoute` больше не вызывается на каждом `pointermove`.
  - Ортогональное перемещение применяется сразу, а починка маршрута выполняется один раз в `pointerup`.
  - При drag устройства `recalcCableRoutes` отложен до `pointerup` (команды `MoveDeviceCommand` / `MoveFreeDeviceCommand` всё ещё выполняют пересчёт при фиксации).

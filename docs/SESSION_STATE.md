...
- В нижней панели инструментов (`src/components/editor/Toolbar.tsx`) справа от кнопки «Выбор» добавлена кнопка «Перенести» с выпадающим меню: «Повернуть», «Обрезать», «Удлинить». В `ToolName` добавлены соответствующие имена, в `PlanEditor.tsx` зарегистрированы заглушки инструментов.

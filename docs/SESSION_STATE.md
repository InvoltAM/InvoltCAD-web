...
- `src/components/editor/SheetsBar.ts`:
  - Меню «Штамп» содержит поля: Стадия, Лист, Листов, Дата, Утвердил, Н. контр., ГИП, Проверил, Согласовал, Разработал.
...
1. Добавить отрисовку оставшихся граф основной надписи (№ документа, организация, объект, название, масса, масштаб) в правильных зонах штампа.
2. Доработать позиционирование и размер шрифта для длинных значений в ячейках штампа.
3. Ручное тестирование в браузере (скриншоты Playwright временно не сохраняются из-за прерывания потока провайдера).

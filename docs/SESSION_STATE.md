...
Последний коммит: `e658eb9` — `ui(cables): widen marking field and panel`.
...
- `src/components/editor/SheetCablesPanel.tsx` (новый компонент):
  - Отображает кабели активного листа в одну строку: **[флажок] | Маркировка | Марка | Длина**.
  - Колонка **Маркировка** шире (1.5fr), чтобы помещались обозначения типа `КГВВГнг(А)-FRLS`.
...
- `src/app/globals.css`:
  - Панели `sheetCables` задана ширина **360px** (320px в компактном режиме).
...
- Проверено в браузере: панель **Кабели** шириной 360px, маркировка `КГВВГнг(А)-FRLS` помещается в поле; одна строка на кабель: флажок, маркировка, марка, длина.

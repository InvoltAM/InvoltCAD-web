...
- В `packages/core/snap/SnapEngine.ts` добавлена привязка к точкам примитивов рисования:
  ...
  - вершины полилинии (`polyline`),
  - углы и центр прямоугольника (`rectangle`),
  - центр круга (`circle`).
- Привязка работает при отрисовке новых примитивов: второй отрезок может начинаться точно с конца первого.
- Процесс рисования примитивов ранее был переделан на два клика (не растягиванием).
- Удалён временный E2E-тест `e2e/temp_primitive_snap.spec.ts`.

...
Последний коммит: `794f32c` — `ui(cables): add cross-section, length to tenths, wider brand field`.
...
- `src/components/editor/SheetCablesPanel.tsx` (новый компонент):
  - Отображает кабели активного листа в одну строку: **[флажок] | Маркировка группы | Марка | Сечение | Длина**.
  - Длина округляется до десятых (например, `5.8 м`).
  - Колонка **Марка** шире, чтобы помещались обозначения типа `КГВВГнг(А)-FRLS`.
...
- Проверено в браузере: панель **Кабели** шириной 400px; одна строка на кабель: флажок, маркировка группы, марка, сечение, длина до десятых; марка `КГВВГнг(А)-FRLS` помещается.

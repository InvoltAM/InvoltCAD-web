...
Последний коммит: `d7301e1` — `feat(cables): separate per-sheet Кабели panel from Кабельный журнал`.
...
- `src/components/editor/SheetCablesPanel.tsx` (новый компонент):
  - Отображает кабели активного листа: тип + сечение, длина, флажок видимости, поля **Марка** и **Маркировка**.
...
- Проверено в браузере: в меню **Панели** отдельно **Кабельный журнал** и **Кабели**; панель **Кабели** показывает кабели текущего листа с видимостью, маркой, маркировкой и длиной; выделение строки подсвечивает кабель.

...
- `packages/core/render/SheetFrameRenderer.ts`:
  ...
  - Добавлено поле **Компания**:
    ...
    - Исправлено закрытие меню «Штамп» при изменениях плана: `SheetsBar.refresh()` теперь не пересоздаёт панель, если открыто меню, поэтому file input и другие контролы сохраняют состояние.

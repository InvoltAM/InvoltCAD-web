...
Последний коммит: будет обновлён после сохранения поэтапной сборки штампа.
...
- `packages/core/render/SheetFrameRenderer.ts` переписан пошагово с нуля:
  ...
  - Итоговая структура полностью соответствует сетке формы 3: левая группа изменений + основное поле с зонами (1)–(4) и правой зоной стадии/листа/листов/масштаба.

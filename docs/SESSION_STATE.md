...
Последний коммит: `refactor(sheet-frame): штамп по строгим параметрам формы 3`.

## Что сделано в этой сессии

### Отрисовка штампа по строгим параметрам формы 3
- `packages/core/render/SheetFrameRenderer.ts`:
  - Габарит штампа строго **185×55 мм**.
  - Внешняя рамка штампа **0,7 мм**, внутренние линии **0,35 мм**.
  - Сетка столбцов: **23, 25, 17, 23, 15, 17, 15, 17, 33 мм** (сумма 185 мм).
  - Сетка строк: **7, 7, 5, 5, 5, 5, 5, 8, 8 мм** (сумма 55 мм).
  - Объединения ячеек:
    - Строки 1–2: столбцы 1–4 — «Наименование изделия»; столбцы 5–9 — «Лист» / «Листов».
    - Строки 3–7: столбцы 1–4 — «Обозначение»; столбцы 5–9 — графы изменений (Изм., Лист, № докум., Подп., Дата).
    - Строки 8–9: столбцы 1–4 — подписи (Разраб., Пров., Н. контр., Утв.); столбцы 5–9 — «Масштаб».
  - Все надписи строго **3,5 мм**, sans-serif, по центру ячеек.
  - Координаты внутри штампа задаются в мм (аналог viewBox="0 0 185 55").
- `packages/core/render/SheetFrameRenderer.ts`:
  - Штамп 185×55 мм.
  - Левая группа: 10+10+10+10+15+10 = 65 мм, 11 строк по 5 мм.
  - Основное поле: 70 мм левая часть + 50 мм правая часть = 120 мм.
  - Верхняя зона (1) — единая на всю ширину 120 мм, без вертикальной перегородки.
  - Вертикальная граница левой/правой части основного поля проведена с y+10 до y+50.

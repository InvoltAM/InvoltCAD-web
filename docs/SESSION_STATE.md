...
- `packages/core/render/SheetFrameRenderer.ts`:
  ...
  - Добавлено поле **Компания**:
    ...
    - В меню «Штамп» добавлены пункты **Компания** (текст) и **Логотип** (кнопка «Загрузить» через скрытый `<input type="file">` + превью + удаление + чекбокс видимости).
    - Исправлено закрытие меню «Штамп» при выборе файла: теперь обработчик внешнего клика использует `mousedown` вместо `click`, поэтому меню остаётся открытым после закрытия системного диалога выбора файла.

...
> **Статус:** production-ready платформа; **штамп приведён к виду по ГОСТ Р 21.101-2020, форма 3** (добавлена подпись формата, убраны лишние графы, исправлены позиции элементов); **в кабельном журнале добавлена колонка «Прол.» с флажком «проложен по факту»** (состояние сохраняется в кабель, отображается в панели и в печатной версии); **исправлено выделение рамкой — теперь выделяются все объекты, попавшие в рамку (window/crossing)**; панель «Кабельный журнал» переведена в плавающую `PanelManager`-панель с drag/resize; dev-сервер перезапущен и работает на `http://localhost:3003/editor`; последний коммит `d8b7fc6`; остальные фичи см. в `docs/SESSION_STATE.md`.

...
> **Статус:** production-ready платформа; **штамп приведён к виду по ГОСТ Р 21.101-2020, форма 3** (добавлена подпись формата, убраны лишние графы, исправлены позиции элементов); **в кабельном журнале добавлена колонка «Прол.» с флажком «проложен по факту»**; **исправлено выделение рамкой — мультивыделение**; **рамка листа со штампом зафиксирована относительно мирового начала координат (0,0)**; панель «Кабельный журнал» переведена в плавающую `PanelManager`-панель с drag/resize; dev-сервер работает на `http://localhost:3003/editor`; последний коммит будет обновлён после фиксации рамки; остальные фичи см. в `docs/SESSION_STATE.md`.

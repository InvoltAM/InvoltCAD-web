...
> **Статус:** production-ready платформа; **штамп приведён к виду по ГОСТ Р 21.101-2020, форма 3**; **в кабельном журнале добавлена колонка «Прол.» с флажком «проложен по факту»**; **исправлено выделение рамкой — мультивыделение**; **рамка листа со штампом зафиксирована относительно мирового начала координат (0,0)**; **инструменты «Дверь» и «Окно» перенесены в выпадающее меню «Стена»**; панель «Кабельный журнал» переведена в плавающую `PanelManager`-панель с drag/resize; dev-сервер работает на `http://localhost:3003/editor`; последний коммит будет обновлён после переноса Двери/Окна; остальные фичи см. в `docs/SESSION_STATE.md`.

## Краткое резюме последней сессии

- Инструменты «Дверь» и «Окно» перенесены из основного dock в выпадающее меню инструмента «Стена»: активный пункт подсвечивается, подменю позиционируется над кнопкой и не смещается при изменении масштаба UI.
- Штамп (основная надпись) доведён до соответствия ГОСТ Р 21.101-2020, форма 3:
  - в нижнем поле справа добавлена подпись формата (`Формат A4 альбомный` и т.д.);
  - убраны неиспользуемые графы «Масса» и «Масштаб»;
  - в SVG-запасном варианте исправлены вертикальные позиции всех заполняемых полей и подписей.
- В кабельный журнал добавлена колонка «Прол.» с флажком: состояние «проложен по факту» сохраняется в кабеле (`Cable.laid`), отображается в панели и в печатной версии.
- Исправлено выделение рамкой: реализовано мультивыделение, теперь выделяются все объекты, попавшие в рамку (window — слева направо, crossing — справа налево); устройства и проёмы выделяются по реальным границам, а не только по центральной точке.
- Рамка листа со штампом зафиксирована: теперь её центр всегда в мировой точке `(0,0)` и не смещается при добавлении/перемещении стен. `fitToSheet` также центрирует камеру на `(0,0)`.
- `npm test` — 39/39.
- Dev-сервер перезапущен и работает на `http://localhost:3003/editor` (PID `19392`).

Подробное состояние и история сессий — в `docs/SESSION_STATE.md`.
